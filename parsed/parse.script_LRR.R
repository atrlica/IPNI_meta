...
### perform calcs for effect sizes, summary effect, significance
...
for (b in 1:length(treats)){
  ...
  set$LRR <- log(set$mean.trt/set$mean.ctrl)  ### log response ratio (LRR) as effect size
...
if(Nlim=="on"){
  write.csv(results, file=paste(ferts[f],"meta.split.LRR-results.Nlim.csv"))
  write.csv(pairs.final, file=paste(ferts[f],"pairs-LRR.Nlim.csv"))
}
else{
  write.csv(results, file=paste(ferts[f],"meta.split.LRR-results.csv"))
  write.csv(pairs.final, file=paste(ferts[f],"pairs-LRR.csv"))
}
