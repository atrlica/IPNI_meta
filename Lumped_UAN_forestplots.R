...
UAN.dat$Inhibitor <- as.character(UAN.dat$Inhibitor)
UAN.dat$Name <- paste(UAN.dat$Study, UAN.dat$Inhibitor) ## mark each study effect size with its Inhibitor

### this loop comes up with a scaling factor for the squares to apply within each Inhibitor group
inhib <- unique(UAN.dat$Inhibitor)
shit <- data.frame(NULL)
for(j in 1:length(inhib)){
  gunk <- UAN.dat[which(UAN.dat$Inhibitor==inhib[j]),]
